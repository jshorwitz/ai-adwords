...
class MultiPlatformETLPipeline:
    ...
    async def _sync_reddit_data(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for account in accounts:
                ...
                logger.info(f"Syncing Reddit account: {account_name} ({account_id})")
                ...
                for campaign in campaigns:
                    ...
                    metrics_data = await client.get_campaign_metrics(
                    ...
                    for date_str, metrics in metrics_data.items():
                        record = self._transform_reddit_metrics(
                            date_str, account_id, account_name, 
            ...
            if all_records:
                ...
                logger.warning("⚠️ No Reddit data found for date range")
                return {
                    "platform": "reddit",
                    "status": "success",
                    "records_written": 0,
    ...
    async def _sync_microsoft_data(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            accounts = await client.get_accounts()
            if not accounts:
                raise ValueError("No Microsoft Ads accounts found")
            
            all_records = []
            
            for account in accounts:
                account_id = account.get("Id", account.get("id"))
                account_name = account.get("Name", account.get("name", f"Account {account_id}"))
                
                logger.info(f"Syncing Microsoft account: {account_name} ({account_id})")
                
                # Get campaigns for this account
                campaigns = await client.get_campaigns(account_id)
                
                for campaign in campaigns:
                    campaign_id = campaign.get("Id", campaign.get("id"))
                    campaign_name = campaign.get("Name", campaign.get("name", f"Campaign {campaign_id}"))
                    
                    # Get performance report for date range
                    metrics_data = await client.get_campaign_performance_report(
                        account_id, campaign_id, start_date, end_date
                    )
                    ...
                    for metrics in metrics_data:
                        record = self._transform_microsoft_metrics(
                            metrics.get("TimePeriod", start_date),
            ...
            if all_records:
                ...
                logger.warning("⚠️ No Microsoft data found for date range")
                return {
                    "platform": "microsoft", 
                    "status": "success",
                    "records_written": 0,
    ...
    async def _sync_linkedin_data(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """Sync LinkedIn Ads data to BigQuery (currently mock implementation)."""
        logger.info("🔗 Starting LinkedIn Ads data sync...")
        
        try:
            # TODO: Implement actual LinkedIn Marketing API client
            # For now, generate mock data that follows the unified schema
            
            all_records = []
            
            # Generate mock LinkedIn data for the date range
            ...
            while current_date <= end_date_obj:
                ...
                mock_record = {
                    ...
                    "revenue": round((8 + (current_date.day * 0.5)) * 125.0, 2),  # $125 per conversion
